...
const SOUNDFONTS: Record<string, { url: string; gain: number }> = {
  FluidR3_GM: {
...
function getSoundfontName(): string {
  try {
...
function getVolumePercent(): number {
  try {
...
let instrumentPromise:
  ...
  | undefined;
let instrumentUrl: string | undefined;
...
export async function playNoteSequence(
  noteNames: Array<string | null | undefined>,
  { bpm = 120 }: { bpm?: number } = {},
): Promise<{ stop: () => void }> {
