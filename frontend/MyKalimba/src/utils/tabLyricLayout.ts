export function isLikelyNotationLine(line: unknown): boolean {
...
export type LyricNotationBlock = {
  lyric: string;
  notes: string;
};

export function buildLyricNotationBlocks(
  content: unknown,
): LyricNotationBlock[] {
  ...
  const blocks: LyricNotationBlock[] = [];
...
export function extractLyricsText(content: unknown): string {
...
export function toContentString(maybeContent: unknown): string {
