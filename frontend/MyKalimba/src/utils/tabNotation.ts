...
export type TabToken =
  ...
  | { type: "bar"; raw: "|" }
  | { type: "newline" }
  | { type: "hold"; raw: string; len: number }
  | { type: "rest"; raw: string; len: number }
  | { type: "note"; raw: string; noteName: string; len: number };
...
function normalizeSymbols(text: unknown): string {
...
function midiToNoteName(midi: unknown): string | null {
...
function splitTokenByHyphen(token: string): string[] {
...
function parseSustainSuffix(raw: string): number {
  const m = normalizeSymbols(raw).match(/(-+)$/);
  return m ? m[1].length : 0;
}

function stripSustainSuffix(raw: string): string {
...
function parseNumberPiece(piece: string): ParsedPiece | null {
...
function parseLetterPiece(piece: string): ParsedPiece | null {
...
function isQuickHyphenChain(
  token: string,
  parsePiece: (piece: string) => ParsedPiece | null,
): boolean {
...
function isStandaloneHoldToken(token: string): boolean {
  return /^-+$/.test(normalizeSymbols(token).trim());
}

export function parseTabContent(
  content: unknown,
  labelType?: string,
): ParseTabResult {
...
export function tokensToNoteSequence(
  tokens: readonly TabToken[],
): Array<string | null> {
