...
class Kalimba_Online {
  _kalimba: {
    play: (note: string, time?: number, options?: { gain?: number }) => void;
  } = {
    play: () => {},
  };
...
function initKalimbaUi() {
  ...
  // Use delegated event handlers so clicks keep working even if React remounts
  // and replaces these button elements (e.g., React.StrictMode dev behavior).
  $(document).off("click.kalimba", "#recordButton");
  $(document).on("click.kalimba", "#recordButton", function () {
    if (isRecording) {
      isRecording = false;
      $("#icon-record").show();
      $("#icon-spin").hide();
  ...
  $(document).off("click.kalimba", "#playButton");
  $(document).on("click.kalimba", "#playButton", function () {
    if (isPlaying) {
      isPlaying = false;
